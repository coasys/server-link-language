...
```bash
NODE_ENV=development pnpm install     # devDependency crypto libs get bundled, not shipped as runtime deps
deno run --allow-all esbuild.ts       # -> build/bundle.js
node --experimental-vm-modules --import tsx --test tests/*.test.ts
npx tsc --noEmit
```

`AD4M_LDK_ENTRY` env var overrides the default `../ad4m/ad4m-ldk/js/lib/index.js`
resolution (both in `esbuild.ts` and `tsconfig.json`'s `paths`).
